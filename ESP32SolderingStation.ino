...
long nextSound = 0;     // переменная для звуковых сигналов
    ...
    volatile uint64_t last_zcc_time = 0;
...
volatile bool DimmingDetected_buf2 = false;
//---------------------------------------------------------------------------
...
void update_aliasprofile(int arg_cnt, char **args);
  ...
  {
     #if defined(ESP8266) || defined(ESP32)
       uint64_t current_time = esp_timer_get_time(); // Время в микросекундах с момента старта
      
       // Если с момента прошлого срабатывания прошло меньше 2.5 мс — игнорируем
       if (current_time - last_zcc_time < DEBOUNCE_TIME_US) {
        return;
       }
    
       last_zcc_time = current_time;
     #endif
...
{
  // Вывод данных в UART
  #ifdef Enable_Serial_Port_Plotter
    if (DimmingDetected) {
      DimmingDetected = false; 
      Serial.println(buf);
    }
    if (DimmingDetected_buf2) {
      DimmingDetected_buf2 = false; 
      Serial.print(buf2);
    }
  #endif
...
void Dimming() // Вызывается по прерыванию от детектора ноля 100 раз в секунду
{
  ...
  if (Secs >= 100) // каждые 100 прерываний отправляем данные через serial
  {
    ...
    Secs = 1; // Начинаем считать с 1, т.к. в первый шаг мы только присваиваем значение, но не прибавляем.7 of 8
...
void UNBLOCK(int arg_cnt, char **args)
{
  ...
  else
  {
    if (reflowState == REFLOW_STATE_IDLE)
    {
      ...
      #elif defined(ESP8266) || defined(ESP32)
         #ifndef interruptTimer
           attachInterrupt(ZCC_PIN, INT0_ISR, FALLING);  // Additional transfer problem prevention
         #else
           timerStart(Timer0_Cfg);
           timerWrite(Timer0_Cfg, 0);
         #endif
