...
#define Enable_Serial_Port_Plotter 1
...
/**
1 : MAX6675
2 : MAX31865 with Pt1000 2WIRE / 4WIRE
3 : MAX31865 with Pt1000 3WIRE
*/

#define TEMP_SENSOR_TOP 1
#define TEMP_SENSOR_BOTTOM 2
#define TEMP_SENSOR_PCB 1
...
// Секция пинов подключеня----------------------------------------
    ...
    #define DEBOUNCE_TIME_US 2500 // Слепое окно 2.5 миллисекунды ZCC
    ...
    byte buzzerPin = 2;        // пин пищалки
    //byte cancelSwitchPin = 21; // пин кнопки отмена или назад

    // назначаем пины усилителя термопары MAX6675 "ВЕРХНЕГО" нагревателя   clk=sck cs=cs do=so
    byte thermoCLK1 = 12; //=sck
    byte thermoCS1 = 4;  //=cs //separately
    byte thermoDO1 = 13;  //=so
    byte thermoMOSI1 = 11;  //=si

    // назначаем пины усилителя термопары MAX6675 "НИЖНЕГО" нагревателя clk=sck cs=cs do=so
    byte thermoCLK2 = 12; //=sck
    byte thermoCS2 = 14;  //=cs //separately
    byte thermoDO2 = 13;  //=so
    byte thermoMOSI2 = 11;  //=si

    // назначаем пины усилителя термопары MAX6675 "ПЛАТЫ" clk=sck cs=cs do=so
    byte thermoCLK3 = 12; //=sck
    byte thermoCS3 = 5;  //=cs //separately
    byte thermoDO3 = 13;  //=so
    byte thermoMOSI3 = 11;  //=si
